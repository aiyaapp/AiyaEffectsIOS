...
dispatch_queue_attr_t AYYEGPUImageDefaultQueueAttribute(void)
{
...
@implementation AYYEGPUImageContext
...
{
    _contextQueue = dispatch_queue_create("com.aiyaapp.AYYEGPUImage", AYYEGPUImageDefaultQueueAttribute());
    
    CFStringRef specificValue = CFSTR("AYYEGPUImageQueue");
...
- (AYYEGLProgram *)programForVertexShaderString:(NSString *)vertexShaderString fragmentShaderString:(NSString *)fragmentShaderString;
{
    ...
    AYYEGLProgram *programFromCache = [shaderProgramCache objectForKey:lookupKeyForShaderProgram];

    if (programFromCache == nil)
    {
        programFromCache = [[AYYEGLProgram alloc] initWithVertexShaderString:vertexShaderString fragmentShaderString:fragmentShaderString];
...
- (EAGLContext *)createContext;
{
    ...
    NSAssert(context != nil, @"Unable to create an OpenGL ES 2.0 context. The AYYEGPUImage framework requires OpenGL ES 2.0 support to work.");
    return context;
}
...
- (AYYEGPUImageFramebufferCache *)framebufferCache;
{
    if (_framebufferCache == nil)
    {
        _framebufferCache = [[AYYEGPUImageFramebufferCache alloc] initWithContext:self];
    }
