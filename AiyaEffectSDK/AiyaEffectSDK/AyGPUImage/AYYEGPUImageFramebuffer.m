...
@interface AYYEGPUImageFramebuffer()
{
    GLuint framebuffer;
...
void dataAYYEProviderReleaseCallback (void *info, const void *data, size_t size);
void dataAYYEProviderUnlockCallback (void *info, const void *data, size_t size);

@implementation AYYEGPUImageFramebuffer
...
- (void)destroyFramebuffer;
{
    runAYYESynchronouslyOnContextQueue(self.context, ^{
        ...
        if ([AYYEGPUImageContext supportsFastTextureUpload] && !_missCVPixelBuffer)
        {
            if (renderTarget)
            {
...
- (void)lockForReading
{
    if ([AYYEGPUImageContext supportsFastTextureUpload])
    {
        if (readLockCount == 0)
        {
...
- (void)unlockAfterReading
{
    if ([AYYEGPUImageContext supportsFastTextureUpload])
    {
        NSAssert(readLockCount > 0, @"Unbalanced call to -[AYYEGPUImageFramebuffer unlockAfterReading]");
        readLockCount--;
        if (readLockCount == 0)
        {
